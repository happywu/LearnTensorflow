...
def RNN(_X, _istate, _weights, _biases):
    ...
    # before transpose _X:(batch_size * n_steps * n_input)
    # after transpose _X:(n*steps * batch_size * n_input)
    ...
    # 开始跑RNN那部分
    _istate = lstm_cell.zero_state(batch_size, tf.float32)
    outputs, states = tf.nn.rnn(lstm_cell, _X, initial_state=_istate)

    # 输出层
    return tf.matmul(outputs[-1], _weights['out']) + _biases['out'] # 因为是做图像分类，所以只用取最后一次的输出，outputs[-1]
